...
async function ingestSource(src: SourceRow, c: PoolClient) {
  ...
  // Compute the URL we’ll fetch: RSS if present, else a list page (scrape_path), with
  // a FantasyPros fallback to their general NFL feed.
  ...
  // Fetch & normalize the feed (with paywall filtering)
  let items: FeedItem[];
  try {
    items = await fetchFeed({
      url: feedUrl,
      dropPremium: true,
      denyPatterns: [...GLOBAL_DENY],
    });
  } catch (e: unknown) {
    return { id: src.id, name: src.name, inserted: 0, error: errorMessage(e) };
  }

  // Upsert items into your articles schema
  let inserted = 0;
  for (const it of items) {
    ...
    const topics: string[] = src.category ? [src.category] : [];
    ...
    try {
      const res: QueryResult = await c.query(
        ...
        VALUES (
          ...
          $3,                 -- title
          $4,                 -- url
          NULL,               -- canonical_url (set later if you normalize)
          $5,                 -- published_at
          NOW(),              -- discovered_at
          $6,                 -- domain
          $7                  -- topics text[]
