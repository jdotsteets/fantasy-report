...
export default async function AdminSourcesPage({
  ...
  // id -> category for team filtering
  ...
  // Row type for the source-level summary table
  type PerSourceIngestRow = NonNullable<HealthSummary["perSourceIngest"]>[number] & {
    category?: string | null;
  };

  // Enrich per-source ingest rows with category for filtering — no `any`
  const perSourceIngest: PerSourceIngestRow[] = (summary.perSourceIngest ?? []).map((r) => ({
    ...r,
    category: catById.get(r.source_id) ?? null,
  }));
  ...
  return (
    <main className="mx-auto max-w-[1100px] space-y-8 px-4 py-8">
      ...
      <section className="rounded-xl border p-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          <form method="GET" className="flex items-center gap-2">
            ...
            <select name="window" defaultValue={String(windowHours)} className="h-8 rounded border px-2 text-sm">
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
          <SummaryTile label="Generated" value={new Date(summary.generatedAt).toLocaleString()} mono />
          <SummaryTile
            label="Most recent"
            value={summary.mostRecent ? new Date(summary.mostRecent).toLocaleString() : "—"}
            mono
          />
        </div>
      </section>
      ...
      <section className="rounded-xl border p-4">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-lg font-semibold">Source-level Summary (last {summary.windowHours}h)</h2>
          ...
          <form method="GET" className="flex items-center gap-3 text-sm">
            ...
            <label className="flex items-center gap-2">
              <input type="checkbox" name="hideTeams" value="1" defaultChecked={hideTeams} />
              Hide team sources
            </label>
        ...
        <SourceLevelSummaryTable rows={perSourceIngest} windowHours={summary.windowHours} />
      ...
      <section className="rounded-xl border p-4">
        ...
        <p className="mt-3 text-xs text-zinc-500">
          Uses site-vs-DB newest-article comparison when available; otherwise falls back to window
          activity. Errors surface HTTP/visibility issues.
        </p>
      </section>
...
function SummaryTile({
  ...
  return (
    <div className="rounded border border-emerald-200 bg-white p-3">
      ...
      <div className={`mt-1 text-lg font-semibold ${mono ? "font-mono" : ""}`}>{String(value)}</div>
    </div>
  );
}
