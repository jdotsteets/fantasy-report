// app/admin/social/page.tsx  (or wherever this file lives)
...
async function getQueue(): Promise<SocialQueueRow[]> {
  const rows = await dbQueryRows<SocialQueueRow>(
    `select * from v_social_queue
     order by status, scheduled_for nulls last, created_at desc
     limit 200`
  );
  return rows;
}
    ...
    <main className="mx-auto max-w-5xl px-4 py-8">
      ...
      <header className="mb-4">
        ...
        <p className="mt-1 text-sm text-zinc-600">
          Drafts, approvals, schedules, and recently published posts.
        </p>
      </header>

      {/* Table / List */}
      {rows.length === 0 ? (
        <div className="rounded-md border p-6 text-sm text-zinc-600">
          No items in the queue yet.
        </div>
      ) : (
        <AdminSocialQueue rows={rows} />
      )}
    </main>
  );
}
