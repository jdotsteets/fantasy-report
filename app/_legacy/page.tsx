...
const inRange = (d: Date, start: MonthDay, end: MonthDay) => {
...
function getSeasonMode(now: Date): SeasonMode {
  // Adjust ranges as needed year-to-year
...
const FREE_AGENCY_RX =
  /\b(free\s+agency|sign(?:ed|ing)?|re-?sign(?:ed|ing)?|trade(?:d|s)?|cut|release(?:d)?|cap\s+hit|contract|extension|tagged|franchise\s+tag|waived|claimed)\b/i;
const DRAFT_RX =
...
export default async function Page({
  ...
  const isSingleSection = selectedSection !== null;

  // Wider window + bigger limits when filtering
  const days = isFilterMode ? 365 : DEFAULT_DAYS;
  const limits = {
    limitNews: isFilterMode ? 150 : isSingleSection ? 20 : 8,
    limitRankings: isFilterMode ? 60 : isSingleSection ? 16 : 6,
    limitStartSit: isFilterMode ? 60 : isSingleSection ? 16 : 6,
    limitAdvice: isFilterMode ? 60 : isSingleSection ? 16 : 6,
    limitDFS: isFilterMode ? 60 : isSingleSection ? 12 : 6,
    limitWaivers: isFilterMode ? 60 : isSingleSection ? 16 : 6,
    limitInjuries: isFilterMode ? 60 : isSingleSection ? 12 : 6,
    limitHero: isFilterMode ? 24 : 10,
  };
...
const showHero = selectedSection === null && hero !== null;
  ...
  type HomeSection = {
    id: string;
    title: string;
    sectionKey: "rankings" | "start-sit" | "waiver-wire" | "dfs" | "injury" | "advice" | "news";
  ...
  const seasonalSection: HomeSection = (() => {
    if (seasonMode === "draft") {
      const draftItems = latestFiltered.filter((a) => {
    ...
    if (seasonMode === "free-agency") {
      const faItems = latestFiltered.filter((a) => {
        const hay = `${a.title ?? ""} ${a.canonical_url ?? a.url ?? ""}`;
    ...
    return {
      ...
      sectionKey: "waiver-wire" as const,
  ...
  const sections: { left: HomeSection[]; middle: HomeSection[]; right: HomeSection[] } = {
    left: [
      {
        id: "rankings",
        title: "Rankings",
        sectionKey: "rankings" as const,
  ...
  return (
    <Suspense fallback={null}>
      <main className="mx-auto w-full max-w-7xl px-3 sm:px-4 lg:px-8 pt-3 pb-6">
        <JsonLd json={baseLd} />
        ...
        {selectedSection ? (
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-[1fr_1.25fr_1fr] md:gap-5">
            {/* Left column */}
            <div className="order-2 md:order-1 space-y-4">
              {sections.left.map((section) => (
                <LoadMoreSection
                  ...
                  initialDisplay={section.initialDisplay}
                  week={section.week}
                  sourceId={selectedSourceId ?? undefined}
                  provider={selectedProvider ?? undefined}
                />
